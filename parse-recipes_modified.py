...
for recipeId in range(0,280000):
    soup = None
    try:
    ...
    if soup:
        ...
        titleSpan = soup.find("h1", class_="headline heading-content")
        if titleSpan is not None:

            directionObjects = soup.find_all("div", class_="paragraph")
            ingredientObjects = soup.find_all("span", class_="ingredients-item-name")
        ...
        for i in range(0, count):
            ...
            ingredientString = ingredientString.replace("\r"," ")
            ...
            while len(parsedIngredient) > 0:
                ...
                try:
                    ...
                    try:
                        ...
                        ingredient["amount"] += unicodedata.numeric(parsedIngredient[0])
                        del parsedIngredient[0]
                    except (ValueError, TypeError):
                        #print('there is fraction')
                        pass
        ...
        directions = []
        for i in range(0, len(directionObjects)):
            direction = {}
            direction["step"] = i
            directionString = directionObjects[i].text
            directionString = directionString.replace("\n","\r")
            directionString = directionString.replace("\r","")
